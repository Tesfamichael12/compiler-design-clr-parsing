...
export class LR1Item {
  ...
  constructor(production: Production, dotPosition: number, lookahead: Symbol) {
      ...
      if (B && B.type === SymbolType.NON_TERMINAL) {
        ...
        prodB.forEach((prod) => {
          firstSet.forEach((term) => {
            ...
            const newItem = new LR1Item(
              prod,
              0,
              new Symbol(
                term,
                term === "$" ? SymbolType.END : SymbolType.TERMINAL,
              ),
            );
...
export function goto(
  ...
  items.forEach((item) => {
    ...
    if (next && next.name === symbol.name) {
      movedItems.push(
        new LR1Item(item.production, item.dotPosition + 1, item.lookahead),
      );
    }
  });
...
export function buildCanonicalCollection(
  ...
  while (changed) {
    ...
    for (let i = 0; i < states.length; i++) {
      ...
      symbols.forEach((symName) => {
        ...
        const symObj = state
          .find((it) => it.nextSymbol()?.name === symName)!
          .nextSymbol()!;
  ...
  const collection = new CanonicalCollection(states, transitions);
  console.log(
    "[buildCanonicalCollection] states:",
    states.map((s) => s.map((i) => i.toString())),
  );
  console.log(
    "[buildCanonicalCollection] transitions:",
    Array.from(transitions.entries()).map(([from, map]) => [
      from,
