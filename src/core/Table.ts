...
export function buildParsingTable(grammar: Grammar): ParsingTable {
        ...
        state.forEach(item => {
            ...
            if (next && next.type === SymbolType.TERMINAL) {
                ...
                if (nextState !== undefined) {
                    ...
                    action.get(i)!.set(next.name, { type: ActionType.SHIFT, value: nextState });
                }
            }
            
            // 2. [A -> alpha ., a] -> REDUCE by A -> alpha
            if (!next) {
                if (item.production.lhs.name === grammar.startSymbol.name && item.lookahead.name === '$') {
                    // [S' -> S ., $] -> ACCEPT
                    ...
                    if (existing) {
                        ...
                        console.warn(`Conflict in state ${i} on symbol ${item.lookahead.name}: Existing ${existing.type} vs New REDUCE`);
                    }
    ...
    return { action, goto, states: collection.states };
}
