...
export function parseString(input: string, _grammar: Grammar, table: ParsingTable): { steps: ParseStep[], accepted: boolean, error?: string } {
    ...
    while (true) {
        ...
        if (!action) {
            ...
            steps.push(stepLog);
            return { steps, accepted: false, error: `Syntax Error at '${currentToken}' in state ${topState}. Expected: ${Array.from(table.action.get(topState)?.keys() || []).join(', ')}` };
        }
        ...
        if (action.type === ActionType.SHIFT) {
            ...
            if (nextState === undefined) {
                 return { steps, accepted: false, error: `GOTO Error: No transition from state ${prevState} on symbol ${lhs}` };
            }
